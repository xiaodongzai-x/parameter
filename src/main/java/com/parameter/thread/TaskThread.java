...
public class TaskThread implements Runnable {
    ...
    private static String getParameterJson(JSONObject jsonObject) {
        ...
        try {
            ...
            e.printStackTrace();
            LogCommon.WriteLogNormal("获取参数信息异常：" + e.getMessage(), "TaskThread");
        }
    ...
    public static JSONArray getParameterTask(InitInfo initInfo) {
        ...
        try {
            ...
            e.printStackTrace();
            LogCommon.WriteLogNormal("获取任务表信息异常：" + e.getMessage(), "TaskThread");
        } finally {
    ...
    private void updateTask(String taskID){
        ...
        try {
            ...
            if(num==1){
                LogCommon.WriteLogNormal("更新任务"+taskID+"成功","TaskThread");
            }else {
                LogCommon.WriteLogNormal("更新任务"+taskID+"成功","TaskThread");
            }
        } catch (SQLException throwables) {
            throwables.printStackTrace();
            LogCommon.WriteLogNormal("更新任务"+taskID+"异常:"+throwables.getMessage(),"TaskThread");
        } finally {
            DataBaseUtil.dataBaseCloseS(connection, ps, rs, Integer.parseInt(database.getDataBaseType()));
        }
    }
    ...
    private void insertTaskLog(JSONObject jsonObject,String taskDate,int number,String level){
        ...
        try {
            ...
            LogCommon.WriteLogNormal("开始将下发记录插入到日志表", "TaskThread");
            //任务插入任务表
            database = DataBaseUtil.getDataBase(0);
            connection = DataBaseUtil.getConn(database);
            ...
            ps = connection.prepareStatement(insertCenter);
                ...
                int num = ps.executeUpdate();
            if(num==1){
                LogCommon.WriteLogNormal("日志记录插入成功","TaskThread");
            }else {
                LogCommon.WriteLogNormal("日志记录插入失败","TaskThread");
            }
        } catch (Exception e) {
            e.printStackTrace();
            LogCommon.WriteLogNormal("日志记录插入异常：" + e.getMessage(), "TaskThread");
        } finally {
            DataBaseUtil.dataBaseCloseS(connection, ps, rs, Integer.parseInt(database.getDataBaseType()));
        }
    }
    ...
    @Override
    public void run() {
        LogCommon.WriteLogNormal(Thread.currentThread().getName() + "遍历任务线程已开启...", "TaskThread");
        ...
        while (true) {
            ...
            if (jsonArray == null || jsonArray.size() == 0) {
                LogCommon.WriteLogNormal("任务表暂无任务，休眠10秒", "TaskThread");
                try {
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                try {
                    ...
                    if (flag) {
                        //成功  更新任务表
                        LogCommon.WriteLogNormal("参数" + filekey + "下发到" + serverIP + "成功", "TaskThread");
                        updateTask(taskID);
                    ...
                    e.printStackTrace();
                    LogCommon.WriteLogNormal("参数下发异常：" + e.getMessage(), "TaskThread");
